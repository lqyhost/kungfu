...
WCBackTestStrategyUtil::WCBackTestStrategyUtil(const string& strategyName):
...
bool WCBackTestStrategyUtil::subscribe_market_data(boost::python::list tickers, short source)
{
...
int WCBackTestStrategyUtil::process_callback(long cur_time)
{
    cur_nano = cur_time;
    int count = 0;
...
bool WCBackTestStrategyUtil::insert_callback(long nano, BLCallback& callback)
{
    if (nano > cur_nano)
    {
...
bool WCBackTestStrategyUtil::insert_callback_py(long nano, boost::python::object func)
{
    BLCallback callback = static_cast<BLCallback>(func);
    return insert_callback(nano, callback);
}

long WCBackTestStrategyUtil::get_nano()
{
...
string WCBackTestStrategyUtil::get_time()
{
    return kungfu::yijinjing::parseNano(kungfu::yijinjing::getNanoTime(), time_format);
}

long WCBackTestStrategyUtil::parse_time(string time_str)
{
    return kungfu::yijinjing::parseTime(time_str.c_str(), time_format);
}

string WCBackTestStrategyUtil::parse_nano(long nano)
{
    return kungfu::yijinjing::parseNano(nano, time_format);
}

int WCBackTestStrategyUtil::insert_market_order(short source,
...
int WCBackTestStrategyUtil::insert_limit_order(short source, string instrument_id, string exchange_id, double price, int volume, LfDirectionType direction, LfOffsetFlagType offset)
{
    int rid = get_rid();
    LFInputOrderField order = {};
    strcpy(order.ExchangeID, exchange_id.c_str());
...
int WCBackTestStrategyUtil::insert_fok_order(short source, string instrument_id, string exchange_id, double price, int volume, LfDirectionType direction, LfOffsetFlagType offset)
{
    int rid = get_rid();
    LFInputOrderField order = {};
    strcpy(order.ExchangeID, exchange_id.c_str());
...
int WCBackTestStrategyUtil::insert_fak_order(short source, string instrument_id, string exchange_id, double price, int volume, LfDirectionType direction, LfOffsetFlagType offset)
{
    int rid = get_rid();
    LFInputOrderField order = {};
    strcpy(order.ExchangeID, exchange_id.c_str());
...
int WCBackTestStrategyUtil::req_position(short source)
{
    int rid = get_rid();
...
int WCBackTestStrategyUtil::cancel_order(short source, int order_id)
{
...
void WCBackTestStrategyUtil::set_pos_back(short source, const char* pos_str)
{
