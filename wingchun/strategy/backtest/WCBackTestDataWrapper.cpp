...
#include "WCBackTestDataWrapper.h"
#include "WCBackTestStrategyUtil.h"
#include "Timer.h"
...
WCBackTestDataWrapper::WCBackTestDataWrapper(IWCDataProcessor *processor, WCStrategyUtil* util):
...
void WCBackTestDataWrapper::add_market_data(short source)
{
    ADD_JOURNAL(getMdJournalPair, source);
}

void WCBackTestDataWrapper::add_register_td(short source)
{
    tds[source] = CONNECT_TD_STATUS_ADDED;
    ADD_JOURNAL(getTdJournalPair, source);
}

void WCBackTestDataWrapper::register_bar_md(short source, int min_interval, string start_time, string end_time)
{
    ...
    for (long nano = start_nano; nano <= end_nano; nano += delta)
    {
        if (nano > cur_time)
        {
            if (!inited)
            {
                ...
                BLCallback callback = boost::bind(&WCBackTestDataWrapper::init_bar, this, source, min_interval);
                util->insert_callback(nano, callback);
                inited = true;
            }
            else
            {
                BLCallback callback = boost::bind(&WCBackTestDataWrapper::send_bar, this, source, min_interval);
                util->insert_callback(nano, callback);
            }
        }
    }
...
void WCBackTestDataWrapper::init_bar(short source, int min_interval)
{
    bars[source][min_interval].clear();
}

void WCBackTestDataWrapper::send_bar(short source, int min_interval)
{
...
void WCBackTestDataWrapper::process_bar(const LFMarketDataField* data, short source, long rcv_time)
{
...
void WCBackTestDataWrapper::pre_run()
{
    /** build up reader now */
...
void WCBackTestDataWrapper::stop()
{
    force_stop = true;
}

void WCBackTestDataWrapper::run()
{
    pre_run();
...
void WCBackTestDataWrapper::process_rsp_position(const LFRspPositionField* pos, bool is_last, int request_id, short source, long rcv_time)
{
...
void WCBackTestDataWrapper::process_td_ack(const string& content, short source, long rcv_time)
{
    try
    {
...
void WCBackTestDataWrapper::set_pos(PosHandlerPtr handler, short source)
{
    set_internal_pos(handler, source);
...
void WCBackTestDataWrapper::set_internal_pos(PosHandlerPtr handler, short source)
{
...
PosHandlerPtr WCBackTestDataWrapper::get_pos(short source)
{
...
byte WCBackTestDataWrapper::get_td_status(short source) const
{
...
double WCBackTestDataWrapper::get_ticker_pnl(short source, string ticker, bool include_fee) const
{
...
vector<int> WCBackTestDataWrapper::get_effective_orders() const
{
    vector<int> res;
